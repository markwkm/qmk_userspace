...
#    endif
    // Define layers here, Have not worked out how to have text displayed
    // for each layer. Copy down the number you see and add a case for it
    // below
    char buf[10];
...
#    endif
    ...
    oled_write_P(led_state.num_lock ? PSTR("NUMLOCK") : PSTR("       "),
                 false);
    oled_write_P(led_state.caps_lock ? PSTR("CAPS") : PSTR("    "), false);
    oled_write_P(led_state.scroll_lock ? PSTR("SCLK") : PSTR("    "),
                 false);
    oled_write_P(PSTR("\n"), false);
#    ifdef SSD1306OLED
